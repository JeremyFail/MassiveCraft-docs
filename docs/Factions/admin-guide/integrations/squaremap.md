...
Factions integrates with [SquareMap](https://modrinth.com/plugin/squaremap) to display faction territories on the SquareMap web interface. SquareMap is a minimalistic 2D world map; Factions draws territory as **polygons with holes** (unclaimed areas inside faction land are cut out). Popups show faction information when clicking on territory.

!!! info "Note"

    For more information about SquareMap, visit the [GitHub repository](https://github.com/jpenilla/squaremap) or [Modrinth](https://modrinth.com/plugin/squaremap).

!!! info "Shared configuration"

    Most map-related settings are **shared** across all map integrations (Dynmap, BlueMap, SquareMap, and Pl3xMap). Only `squaremapEnabled` and `squaremapLogTimeSpent` are specific to SquareMap. The shared settings use the `map*` prefix and are documented below; changing them affects all enabled map integrations.
...
### SquareMap-Specific Settings

| Setting | Default | Description |
|---------|---------|-------------|
| `squaremapEnabled` | `true` | Enable/disable the SquareMap integration |
| `squaremapLogTimeSpent` | `false` | Log SquareMap update timing to console (debug) |

### Map Settings (Territory Layer)

These settings apply to Dynmap, BlueMap, and SquareMap. SquareMap is 2D and does not use `mapTerritoryMaxY`.
...
| `mapWarpHomeIcon` | `"redflag"` | Icon for Faction Homes. Note that you cannot use custom images with SquareMap - if the default value of `redflag` is specified, a red circle will be generated. Otherwise, the string specified will be converted to a hex value and used to create a colored icon. You can specify a 6-digit RRGGBB hex value (no # symbol) as the icon name to create a specific color. |
...
| `mapWarpOtherIcon` | `"greenflag"` | Icon for Faction Warps. Note that you cannot use custom images with SquareMap - if the default value of `greenflag` is specified, a green circle will be generated. Otherwise, the string specified will be converted to a hex value and used to create a colored icon. You can specify a 6-digit RRGGBB hex value (no # symbol) as the icon name to create a specific color. |
...
- Check `squaremapEnabled` is `true`
- Verify SquareMap is installed and running
- Check `mapVisibleFactions` and `mapHiddenFactions`
- Ensure factions have claimed territory

**Performance problems:**

- Enable `squaremapLogTimeSpent` to identify bottlenecks
...
Set `squaremapLogTimeSpent` to `true` in the config to log update timing to the console.
